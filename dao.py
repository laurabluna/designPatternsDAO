...
class ProdutoDAO(DAO):
    ...
    def incluir(self, obj:Produto):
        sql = f'''  INSERT INTO Produto(descricao, preco, categoria_id) 
                    VALUES('{obj.descricao}', {obj.preco}, {obj.categoria.id}) '''
        self.executar_sql(sql)
    ...
    def selecionar_todos(self) -> list[Produto]: 
        ...
        sql = '''   SELECT  p.id, p.descricao, p.preco, c.id, c.descricao 
        ...
        dados = list()
        for reg in registros:
            dados.append( Produto(id=reg[0], descricao=reg[1], preco=reg[2], categoria=Categoria(id=reg[3], descricao=reg[4])) )
        # retorna
        return dados

    def selecionar_um(self, id: int) -> Produto: 
        # seleciona o registro pelo id informado
        sql = f'''  SELECT  p.id, p.descricao, p.preco, c.id, c.descricao 
                    FROM Produto p JOIN Categoria c ON p.categoria_id=c.id 
                    WHERE p.id={id} '''
        # executa o select e pega o primeiro registro([0])
        reg = self.executar_select(sql)[0]
        # converte o registro para objeto e retorna
        return Produto(id=reg[0], descricao=reg[1], preco=reg[2], categoria=Categoria(id=reg[3], descricao=reg[4]))
